        ...
        <div class="w-full max-w-lg rounded-2xl border border-bq-border bg-bq-surface shadow-2xl" @click.stop>
            ...
            <form method="POST" action="{{ request()->is('*services*') ? '/owner/services' : '/owner/programs' }}" enctype="multipart/form-data" @submit="sedangkirim = true" id="form-add-program">
                ...
                <div class="flex items-center justify-end gap-3 border-t border-bq-border px-4 sm:px-6 py-3.5 sm:py-4">
                    <button type="button" @click="buka = false" class="flex-1 sm:flex-none justify-center inline-flex rounded-xl border border-bq-border bg-bq-surface px-4 py-2.5 text-xs sm:text-sm font-semibold text-bq-text transition-all hover:bg-bq-background active:scale-95" id="btn-cancel-program">
                        Cancel
                    </button>
                    <button
                        type="submit"
