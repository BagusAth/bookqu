...
>
    ...
    >
        <div class="w-full max-w-lg rounded-2xl border border-bq-border bg-bq-surface shadow-2xl" @click.stop>
            ...
            {{-- Form --}}
            <form method="POST" :action="`{{ request()->is('*services*') ? '/owner/services' : '/owner/programs' }}/${program.id}`" enctype="multipart/form-data" @submit="sedangkirim = true" id="form-edit-program">
                @csrf
                @method('PUT')
